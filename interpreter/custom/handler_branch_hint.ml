...
let starts_before r1 r2 = (r1.left.line < r2.left.line) ||
                    (r1.left.line == r2.left.line &&
                     r1.left.column < r2.left.column)
...
let flatten_instr_locs is =
  let rec flatten is = match is with
  | [] -> []
  | i::rest ->
      ...
      | _ -> [i] in
      group @ (flatten rest) in
  let flat = flatten is in
  let indexed = List.mapi (fun idx i -> (idx, i)) flat in
  let sorter (_,i1) (_,i2) =
    if starts_before i1.at i2.at then -1 else 1 in
...
let decode_hint locs foff s =
  ...
  require (one = 1l) (pos s) "@metadata.code.branch_hint section: missing reserved byte";
  let k = decode_byte s in
  let hint = match k with
  | 0x00 -> Unlikely
  | 0x01 -> Likely
  | _ -> decode_error (pos s) "@metadata.code.branch_hint section: invalid hint value" in
  let abs_off = Int32.to_int (Int32.add off foff) in
  let at = region "" abs_off abs_off in
  let idx = match get_inst_idx locs at with
  | Some i -> i
  | None -> decode_error (pos s) "@metadata.code.branch_hint section: invalid offset" in
  (hint, idx) @@ at
...
let encode_hint locs foff buf h =
  ...
  let i = match get_nth_inst locs idx with
  | Some(i) -> i
...
let merge_func_hints = IdxMap.merge (fun key x y ->
  ...
  | Some a, Some [{at=_; it=(_, idx)} as b] ->
      ...
      if last_idx >= idx then
        parse_error b.at "@metadata.code.branch_hint annotation: duplicate annotation"
      else
      Some (a @ [b])
  | Some _, Some _ ->
      assert false
...
let find_func_idx m annot =
  ...
  | None -> parse_error annot.at "@metadata.code.branch_hint annotation: not in a function"
...
and parse_annot m annot =
  ...
  let payload a = match a.it with
    | String s -> s
    | _ -> parse_error a.at "@metadata.code.branch_hint annotation: unexpected token" in
  ...
  | "\x00" -> Unlikely
  | "\x01" -> Likely
  | _ -> parse_error annot.at "@metadata.code.branch_hint annotation: invalid hint value" in
  let at = Source.{left = annot.at.left; right = at_last.right} in
  let hidx = match get_inst_idx locs at with
  | Some i -> i
  | None -> parse_error annot.at "@metadata.code.branch_hint annotation: invalid placement" in
  ...
  | i::rest ->
      ...
      | Sexpr.Node (h, ns) ->
        ...
        if ( String.starts_with ~prefix:"block" h
          ...
          | [Sexpr.Node("result",res); Sexpr.Node(hif, nif); Sexpr.Node(helse, nelse)] ->
              let newif = apply_instrs annots fidx curi nif in
              let newelse = apply_instrs annots fidx curi nelse in
              annot @ [Sexpr.Node(h, [Sexpr.Node("result",res); Sexpr.Node(hif, newif); Sexpr.Node(helse, newelse)])]
          | _ -> assert false
        else
  ...
  | Some i ->
      ...
      | If _ | BrIf _ ->
          if !prev_hidx >= idx then
            check_error h.at "@metadata.code.branch_hint annotation: invalid order"
          else
            begin
              prev_hidx := idx;
              ()
