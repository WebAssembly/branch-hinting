...
let empty = {module_ = None; funcs = IdxMap.empty; locals = IdxMap.empty }
...
let decode _m _bs custom =
  ...
  try
    ...
    {module_; funcs; locals} @@ custom.at
...
let rec parse m bs annots =
  let ms = List.map (parse_annot bs m) annots in
  match ms with
  | [] -> []
  | m::ms' -> [List.fold_left merge (empty @@ m.at) ms]

and parse_annot bs m annot =
...
let check (m : module_) (fmt : format) =
  ...
  IdxMap.iter (fun x map ->
    ...
    let Types.FuncType (ts, _) = func_type_for m f.it.ftype in
